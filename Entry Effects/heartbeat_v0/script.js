...
class HeartbeatTextController {
    ...
    startHeartbeat() {
        const heartbeatDuration = 60000 / this.currentHeartRate; // 毫秒
    ...
    showCharacter(char, priority) {
        ...
        // 添加出現時的脈衝效果
        char.style.animation = 'heartbeatText 0.6s ease-out';
        
        setTimeout(() => {
            char.style.animation = '';
            if (priority === 'high') {
                char.style.animation = 'heartbeatText 1.2s ease-in-out infinite';
            }
        }, 600);
    }
